...
# Make sure the button has the correct onclick
html = re.sub(r'<button type="button" id="btnProcesarJson".*?</button>', '<button type="button" id="btn-procesar-json" class="btn-confirmar" style="width: 100%;" onclick="procesarJsonManual()">PROCESAR JSON</button>', html)
# Let's also check if the old ID was btn-procesar-json without onclick
html = re.sub(r'<button type="button" id="btn-procesar-json" class="btn-confirmar" style="width: 100%;">Procesar JSON</button>', '<button type="button" id="btn-procesar-json" class="btn-confirmar" style="width: 100%;" onclick="procesarJsonManual()">PROCESAR JSON</button>', html)

# Now let's extract the JS and replace it
...
function procesarJsonManual() {
    console.log("¡Función disparada por onclick directo!");
    const textarea = document.querySelector('textarea') || document.getElementById('jsonInput');
    if (!textarea) {
        alert("No se encuentra el campo de texto.");
        return;
    }
    ...
    // Envío por fetch a la vista de Django
    ...
    .then(data => {
        ...
        if (data.status === 'success' || data.cliente) {
            ...
            alert("¡JSON procesado con éxito!");
        } else {
