...
# Now, we carefully strip the {% if success %} wraps.
# 1. Sidebar status card
...
# 3. Main Verification panels
# It starts with {% if success %} before verification-bar
# and ends with {% else %} and then empty state and then {% endif %}
main_content = re.sub(
...
# And remove the {% else %} empty state block
empty_state_block_regex = r'\{%\s*else\s*%\}\s*<!-- ESTADO VAC\u251c\u00ccO -->.*?\{%\s*endif\s*%\}'
main_content = re.sub(empty_state_block_regex, '', main_content, flags=re.DOTALL)
empty_state_block_regex2 = r'\{%\s*else\s*%\}\s*<!-- ESTADO VACÍO -->.*?\{%\s*endif\s*%\}'
main_content = re.sub(empty_state_block_regex2, '', main_content, flags=re.DOTALL)

# But wait, the encoding for `├ì` in `VAC├ìO` might not match. Let's do it safely:
# The else block starts with `{% else %}` right before `<!-- ESTADO VAC`
...
with open('mediciones/templates/mediciones/ocr_lector.html', 'w', encoding='utf-8') as f:
    f.write(full_html)
print('Restored HTML layout properly.')
