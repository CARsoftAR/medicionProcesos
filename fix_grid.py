...
new_table_logic = '''const tbody = document.querySelector("#tabla-ocr-resultados tbody");
                    if (tbody && (paginaDatos.controles || data.controles)) {
                        ...
                        // Encontrar qué piezas necesitamos pintar (hasta la máxima, típicamente de 1 al N)
                        let maxNum = 10;
                        controlesData.forEach(c => {
                            if (c.piezas && typeof c.piezas === 'object') {
                                Object.keys(c.piezas).forEach(k => {
                                    // Parsear la base de la clave (ej "26_1" -> 26)
                                    let match = k.match(/^(\\d+)/);
                                    if (match) {
                        ...
                        // El array de piezas siempre será secuencial del 1 al maxNum
                        let arrayPiezas = Array.from({length: maxNum}, (_, i) => (i + 1).toString());
                        ...
                        controlesData.forEach(row => {
                            ...
                            arrayPiezas.forEach(p => {
                                ...
                                if (row.piezas && typeof row.piezas === 'object') {
                                    // Búsqueda inteligente: buscar coincidencia exacta o base (ej: p="26", busca "26", "26_1", "26_2", etc.)
                                    if (row.piezas[p] !== undefined) {
                                        vItem = row.piezas[p];
                                    } else {
                                        // Buscar una clave que empiece con "p_"
                                        const prefix = p + "_";
                                        const matchingKey = Object.keys(row.piezas).find(k => k.startsWith(prefix));
                                        if (matchingKey) {
                                            vItem = row.piezas[matchingKey];
                                        }
                                    }
                                } else if (row.valores && Array.isArray(row.valores)) {
                                    // Si no hay mapeo objeto, cae al modo secuencial array original
                                    let idx = parseInt(p) - 1;
