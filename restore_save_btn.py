...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (btn) {
        btn.addEventListener("click", function () {
            ...
            .then(data => {
                ...
                window.currentOcrData = paginaDatos; // Guardamos para la exportación final
                ...
                if (paginaDatos.status === "success" || paginaDatos.cliente || data.status === "success") {
                    ...
                    function seleccionarParcial(selectId, textoBuscado) {
                        ...
                        if (!select || !textoBuscado) {
                            console.log(`[Autoseleccion] Saltando ${selectId}: valor nulo o elemento inexistente.`);
                            return;
                        }
                        const limpiar = (str) => String(str).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                        const objetivo = limpiar(textoBuscado);
                        console.log(`[Autoseleccion] Buscando objetivo limpio '${objetivo}' en ${selectId}`);

                        let matchFound = false;
                        for (let i = 0; i < select.options.length; i++) {
                            // Ignorar la opcion por defecto si está vacía
                            ...
                            if (textoOpcion.includes(objetivo) || objetivo.includes(textoOpcion) || valorOpcion === objetivo) {
                                ...
                                console.log(`[Autoseleccion] ÉXITO en ${selectId}: '${select.options[i].text}' coincide con '${textoBuscado}'`);
                                matchFound = true;
                                break;
                            }
                        }
                        if (!matchFound) {
                            console.log(`[Autoseleccion] FRACASO en ${selectId}: Ninguna de las opciones contuvo '${objetivo}'.`);
                        }
                    }
                    ...
                    if (tbody && (paginaDatos.controles || data.controles)) {
                        ...
                        // Generar lista de piezas basada en data.piezas, paginaDatos.piezas o un rango fijo hasta 26
                        let piezas = paginaDatos.piezas || data.piezas;
                        if (!piezas || !Array.isArray(piezas) || piezas.length === 0) {
                            piezas = Array.from({length: 10}, (_, i) => (i + 1).toString());
                        }
                        ...
                        controlesData.forEach(row => {
                            ...
                            piezas.forEach(p => {
                                ...
                                if (row.piezas && row.piezas[p] !== undefined) {
                                    ...
                                    let idx = piezas.indexOf(p);
                                    if(idx !== -1 && idx < row.valores.length) vItem = row.valores[idx] || '';
    ...
    if (btnGuardar) {
        btnGuardar.addEventListener('click', function() {
            ...
            const piezas = window.currentOcrData.piezas || Array.from({length: 10}, (_, i) => (i + 1).toString());
            
            // Inyectar forzosamente lo que el usuario ve o eligió en los selectores
            // Si el select tiene un option sin ID, mandamos el texto de la opción seleccionada o del json original
            ...
            .then(data => {
                if (data.status === 'success') {
                    // Usar alert nativo en caso de que Swal no esté disponible
...
with open('mediciones/templates/mediciones/ocr_lector.html', 'w', encoding='utf-8') as f:
    f.write(html)
print('Restored save button logic correctly.')
