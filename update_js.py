...
# We need to replace the JS block.
        ...
        btn.addEventListener("click", function () {
            ...
            .then(data => {
                ...
                if (data.paginas && Array.isArray(data.paginas) && data.paginas.length > 0) {
                    // Por ahora tomamos la primera página para mostrar
                ...
                if (paginaDatos.status === "success" || paginaDatos.cliente || data.status === "success") {
                    ...
                    function seleccionarValorPorTextoOId(selectId, valorBuscado) {
                        ...
                        let valorStr = valorBuscado.toString().trim().toUpperCase();
                        
                        for (let i = 0; i < select.options.length; i++) {
                            if (select.options[i].value == valorBuscado || select.options[i].text.trim().toUpperCase() === valorStr) {
                    ...
                    if (tbody && paginaDatos.controles) {
                        tbody.innerHTML = "";
                        ...
                        paginaDatos.controles.forEach(row => {
...
with open('mediciones/templates/mediciones/ocr_lector.html', 'w', encoding='utf-8') as f:
    f.write(html)
print('Frontend JS updated properly.')
