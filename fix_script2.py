...
# Replace button ID and add onclick
html = html.replace('id="btn-procesar-json"', 'id="btnProcesarJson" onclick="procesarLectorJson(event)"')

# Now let's extract the JS and wrap it in procesarLectorJson
...
new_script = '''<script>
    ...
    // Usando onclick directo para máxima fiabilidad
    function procesarLectorJson(e) {
        if(e) e.preventDefault();
        console.log('¡Clic detectado correctamente! Iniciando lectura de JSON.');
    ...
    // Por redundancia y compatibilidad, también lo registramos vía listener si es posible
    document.addEventListener('DOMContentLoaded', function() {
        const btn = document.getElementById('btnProcesarJson');
        if (btn) {
            btn.addEventListener('click', function(e) {
                if (!e.defaultPrevented) {
                    if (!btn.getAttribute('onclick')) {
                        procesarLectorJson(e);
                    }
                }
            });
        } else {
            console.error('No se encontró el botón con ID btnProcesarJson en el DOM.');
        }
    });
</script>'''
...
with open('mediciones/templates/mediciones/ocr_lector.html', 'w', encoding='utf-8') as f:
    f.write(html)
print('Direct onclick implemented')
